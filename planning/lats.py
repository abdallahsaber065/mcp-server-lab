...
def _backpropagate(node: Optional[LATSNode], value: float) -> None:
...
def _trajectory_reflections(node: Optional[LATSNode]) -> list[str]:
    path: list[str] = []
    curr: Optional[LATSNode] = node
    while curr is not None:
        path.extend(curr.reflections)
        curr = curr.parent
...
def lats(
    ...
    for iteration in range(1, iterations + 1):
        ...
        proposed: Any = llm.with_structured_output(
            LATSActionBatch,
            method="json_schema",
        ).invoke([
        ...
        actions_list = proposed.actions if isinstance(proposed, LATSActionBatch) else getattr(proposed, "actions", [])
        for item in actions_list[:n_actions]:
            state_val = getattr(item, "state", "").strip() if not isinstance(item, dict) else item.get("state", "").strip()
            action_val = getattr(item, "action", "") if not isinstance(item, dict) else item.get("action", "")
            child = LATSNode(state=state_val, action=action_val, parent=leaf)
            leaf.children.append(child)
            feedback = environment.evaluate(child.state)
            child.feedback = feedback
            child.environment_score = feedback.score

            value_judgment: Any = llm.with_structured_output(
                ValueEstimate,
                method="json_schema",
            ).invoke([
            ...
            child.model_score = float(getattr(value_judgment, "score", 0.5)) if not isinstance(value_judgment, dict) else float(value_judgment.get("score", 0.5))
